# Design Rules: fx-ui Design Re

> 报告页的视觉规范。每节分为**原则**（为什么）和**规格**（是什么）两层。
> CSS 实现细节参考 `example-wrapper.html`；图标脚本参考 `lucide-snippet.html`。
...
- 结构（上→下）：
  - `.sc-num`：40px 900 fg-1，`letter-spacing: -0.03em`
...
- 左侧竖条：`6×12px`，`border-radius: 3px`，`background: var(--primary)`，带光晕 `box-shadow: 0 0 0 3px var(--primary-soft)`，`margin-right: 12px`
...
- 数值：36px 700 `var(--text-1)`，字体 `'Barlow Condensed', 'DIN Alternate', 'Arial Narrow', sans-serif`，`font-variant-numeric: tabular-nums`，`margin-top: 12px`
- 单位 small：15px 600 `var(--text-1)`，字体回退系统中文字体（`'PingFang SC', 'Microsoft YaHei', sans-serif`），`margin-left: 4px`
- 需在 `<style>` 头部引入：`@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@600;700&display=swap')`
...
- 图表卡片：白底，`border-radius: 16px`，`padding: 24px`，`gap: 20px`
...
| `hl-primary` | 品牌橙 | 品牌级强调（慎用）：标题关键词、报告核心主题 | — |
...
.hl-primary { background: var(--primary-soft); color: var(--primary); border-radius: 3px; padding: 1px 4px; font-weight: 600; }
...
**规格**：
- 白底，`border-left: 4px solid var(--primary)`，`border-radius: 16px`，`padding: 20px 24px`

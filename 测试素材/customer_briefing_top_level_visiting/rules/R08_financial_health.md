...
在 `--full` 模式下整理客户历史合作、合同订单、回款应收、客户健康、流失原因和续约/增购信号。该规则服务高层陪访，不做财务审计；重点是判断是否存在领导必须知道的历史包袱、价值机会或经营风险。

## 触发条件

满足任一条件时执行：

- 用户使用 `/escalate [客户名称] --full`。
- 客户为已成交、历史合作、续约、增购、沉默唤醒或流失挽回场景。
- R05 出现回款、交付、服务、客户健康、流失风险。
- 用户明确要求查看合同、订单、回款、续约、增购或客户健康。

标准模式不输出本区块，但如果出现重大回款、交付、投诉或流失风险，应摘要进入 R05。
...
| 层级 | 判断方式 | 陪访含义 |
| --- | --- | --- |
| strategic | 合同/订单金额高、组织影响大、可复制标杆或集团化机会 | 领导出面价值高，需要跨部门协同 |
| growth | 有持续合作、续约或增购空间 | 领导可强化合作信任和扩展机会 |
| nurture | 金额较小但行业、场景或关系有潜力 | 谨慎投入，先确认真实高层意愿 |
| low | 历史金额低、沉默久、增购信号弱 | 不建议仅因历史合作安排领导陪访 |
| unknown | 信息不足 | 进入陪访缺口 |
...
### 合同与订单
| 类型 | 名称 | 金额 | 周期/时间 | 状态 | 回款/应收 | 领导需知道 |
| --- | --- | --- | --- | --- | --- | --- |
| 合同/订单 |  |  |  |  |  |  |

### 客户健康信号
| 信号 | 类型 | 证据 | 对陪访的影响 | 建议处理 |
| --- | --- | --- | --- | --- |

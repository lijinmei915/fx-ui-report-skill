...
当用户要求“HTML 版、网页查看、手机查看、自适应页面、生成 html、导出 html”时，将已生成的团队日报、团队周报或团队月报转换成可在手机端和 Web 端阅读的单页 HTML。

HTML 是展示形态，不改变团队报表的分析逻辑、数据来源优先级和 CRM 日志保存确认规则。
...
如果用户只说“团队周报”，不默认生成 HTML；按 Markdown/文本报表输出。

## 执行顺序

1. 先按 `R01_period_router.md` 识别周期和团队范围。
2. 按对应日报、周报或月报规则生成标准团队报表正文。
3. 读取 `references/html_responsive_design_reference.md`。
4. 优先使用 `assets/team_report_template.html` 作为 HTML 模板。
5. 将团队报表正文映射到模板占位符。
6. 输出完整单文件 HTML。
7. 最后仍按 `R09_team_journal_storage_confirmation.md` 询问是否保存到 CRM 日志。

不要为了生成 HTML 跳过 CRM 查询、风险识别、管理动作生成或数据说明。
...
- 表格必须包在横向滚动容器中；如果输出手机卡片，需与桌面表格内容一致。
- 颜色克制，适合销售主管经营看板；不要做营销页或大幅 hero。
...
`team-report-[period]-[date-or-range].html`

例如：

`team-report-weekly-<YYYY-WXX>.html`

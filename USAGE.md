...
对外 zip 文件名固定为 `fx-ui-report-skill.zip`，这样 `latest` 下载链接可以长期不变。为了让接入方确认自己下载的是哪一版，构建脚本会把版本信息写入分发包里的 `SKILL.md` 顶部 frontmatter：

```yaml
version: "v1.0.4"
updated_at: "2026-06-12"
build_commit: "xxxxxxx"
download_url: "https://github.com/lijinmei915/fx-ui-report-skill/releases/latest/download/fx-ui-report-skill.zip"
```
...
在报告类型尚未稳定前，本 skill 不维护 `sales-report`、`customer-briefing`、`weekly-review` 这类固定页面模板。固定模板会让早期接入更快，但也容易在数据变化时遗漏模块、硬塞空块或误导 Agent 造结构。

当前采用“通用生成协议”：

1. 先把任意输入拆成信息桶：`header`、`summary`、`metrics`、`insights`、`actions`、`people`、`tables`、`charts`、`notes`。
2. 再按信息优先级组织：结论先行 → 关键指标 → 风险/机会/异常 → 行动建议 → 图表/表格支撑 → 人员 → 方法论。
3. 最后按组件映射输出：有数据才出模块，没数据就隐藏；现有组件表达不了但必须保留的内容，用 `generic-block` 兜底。

这套方式适合报告形态还在探索期：它不要求先定义报告类型，但能保证生成速度、结构稳定性和 fx-ui 视觉一致性。

等某类报告连续出现 3 次以上，并且章节顺序、必选模块和数据字段趋于稳定，再升级为“报告类型蓝图”。蓝图仍然只是推荐编排，不是不可变模板。

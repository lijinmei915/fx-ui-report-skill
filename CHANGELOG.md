...
- 浅色模式报告头恢复 `components.html` 的标准柔和渐变，避免被深色适配逻辑影响。
- 深色模式报告头改为“暗底 + 局部主题光晕”方案，去除横向高亮色带，过渡更自然。
- 修复预设/自定义主题切换时 `--header-primary-*` 可能残留旧颜色的问题，并用 `data-fx-mode` 明确区分浅色/深色覆盖规则。
